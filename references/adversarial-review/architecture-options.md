# Architecture Options — Adversarial Review
...
Options A-D trigger on spec file merge to `docs/specs/` on the main branch of your repository. Options E-G are session-triggered. Option H is Linear assignment-triggered (async).
...
The agent receives the spec content, the three reviewer perspective prompts, and outputs a structured review as an issue or PR comment. Quality is solid for catching gaps and ambiguities but may miss subtle architectural concerns.
...
**Phase 2 -- Review (4 personas x 2 tiers = 8 reviews):**
...
> For the canonical agent dispatch architecture (reactivity model, adoption status, dispatch-by-stage, routing tables), see **CONNECTORS.md Section: Agent Dispatch Protocol**. For finding normalization from external agent output, see `references/adversarial-review/finding-normalization.md`.
